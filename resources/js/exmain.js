...
$('input[type=radio][name=geodata]').change(function () {
    ...
    if (this.value == 'old') {
        ...
        $.ajax({
            ...
            .done(function (data) {
                console.log(data)
                // new_lat = $('li.city.list-group-item.active').data('city-new_lat');
                // new_lng = $('li.city.list-group-item.active').data('city-new_lng');
                // console.log(new_lng + ' '+ new_lat);
                // curr_lat = new_lat;
                // curr_lng = new_lng;
                // console.log('new')
                // $('#mapYandex').children().remove();
...
$("#UpdateCity").on("click", function () {
    ...
    if (confirm('Редактировать ?')) {
        ...
        $("li.city.list-group-item.active").attr("data-city-new_lat", $("#newlat").val());
        $("li.city.list-group-item.active").attr("data-city-new_lng", $("#newlng").val());

        // city.attr('data-city-new_lng', $("#newlng").val());
        // city.attr('data-city-new_lat', $("#newlat").val());
